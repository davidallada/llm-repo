...
get_model_lists() {
    ...
    # Parse the JSON file and extract model names and draft models
    local model_names=$(jq -r '.model_configs[].model_name' "$config_file")
    local draft_model_names=$(jq -r '.model_configs[].draft_model // empty' "$config_file")
...
readarray -t base_models < <(get_model_lists "$config_file" | sed '/---/q' | sed '$d')
readarray -t draft_models < <(get_model_lists "$config_file" | sed -n '/---/,$p' | tail -n +2)
